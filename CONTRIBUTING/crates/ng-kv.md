...
> 概览：ng-kv 提供一个基于单张 `kv` 数据库表的命名空间作用域键值存储——每条记录对应一个 key，通过 `namespace` 列区分逻辑命名空间。它在默认特性下仅暴露 `KVStore` 类型（agent 也可安全依赖）；启用 `server` 特性后追加 SeaORM CRUD、RBAC 权限层（read/write/delete/list/create-namespace 校验，依据 `Permission::Kv` + `Scope::KvNamespace`），以及 `kv_*` JSON-RPC 命名空间（8 个方法）。本 crate 无 DB 缓存层，每次操作直接落库。
...
| `auth.rs` | KV 操作的 RBAC 权限层；校验命名空间/key、一次性解析 token 状态，通过内存 `Limit` 匹配 `Permission::Kv` / `Scope::KvNamespace` 完成 read/write/delete/list 校验；super-token 与 token 获取委派给 `ng_token` + 注入的 `PermissionChecker`。 |
...
| 名称 | 签名 | 行为 |
|------|------|------|
| `NAMESPACE_MARKER_KEY` | `pub const NAMESPACE_MARKER_KEY: &str = "__nodeget_namespace_marker__"`（`db.rs:22`） | `create_kv` 写入的标记键（值为 NULL），用于标记命名空间已存在。**保留键名，勿用作真实 key**。 |
...
| `check_kv_read_permission` | `pub async fn check_kv_read_permission(token, namespace, key) -> Result<()>`（`auth.rs:171`） | `validate_namespace`+`validate_key`，解析 token；非 super 需 `Scope::KvNamespace(namespace)` 下 `Kv::Read("*")` 或 `Kv::Read(key)`。 |
...
| `check_kv_write_permission` | `pub async fn ...(token, namespace, key) -> Result<()>`（`auth.rs:277`） | `Kv::Write("*")` 或 `Kv::Write(key)`。 |
| `check_kv_delete_permission` | `pub async fn ...(token, namespace, key) -> Result<()>`（`auth.rs:330`） | `Kv::Delete("*")` 或 `Kv::Delete(key)`。 |
| `check_kv_delete_namespace_permission` | `pub async fn ...(token, namespace) -> Result<()>`（`auth.rs:377`） | 需 `Scope::KvNamespace(namespace)` 下**特定** `Kv::Delete("*")`；逐 key 删除权限不足以删整个命名空间。 |
| `check_kv_list_keys_permission` | `pub async fn ...(token, namespace) -> Result<()>`（`auth.rs:426`） | 需 `Scope::KvNamespace(namespace)` 下 `Permission::Kv(Kv::ListAllKeys)`；无通配替代。 |
...
| `NAMESPACE_MARKER_KEY` | `crates/ng-kv/src/db.rs:22` | `"__nodeget_namespace_marker__"`，`create_kv` 写入的标记键（值 NULL），存在性推断依据。 |
...
| `pub use auth::{...}` | `crates/ng-kv/src/lib.rs:21` | server 再导出权限/校验助手（见上表）。 |
...
命名空间是逻辑概念：存在性由 `kv` 表中该 namespace 的任一行（含 marker）推断。`create_kv` 写入 key=`NAMESPACE_MARKER_KEY`、value=NULL 的行。`set_v_to_kv` **不会**隐式创建命名空间——它调 `ensure_namespace_exists`，否则报错，借此防止 RBAC 通过隐式创建被绕过。marker **未**从 `get_keys_from_kv` / `get_kv_store` 输出中过滤（`get_all_keys` 响应会包含 `__nodeget_namespace_marker__`）。
...
### Token 解析：ng_token vs 注入 PermissionChecker

`auth.rs` 存在两条不同代码路径。多数函数（`resolve_token_for_kv_check`）直接调用 `ng_token::check_super_token` 与 `ng_token::get_token`；而 `check_kv_create_permission` 与 `resolve_kv_list_namespace_permission` 改用 `require_permission_checker()?`（来自 `ng-core`、经 OnceLock 注入的 `PermissionChecker`）。两者最终都解析到 `ng-token` 数据，但风格不一致——新增权限函数时必须择一而从。
...
每个 `KvRpcImpl` 方法调 `token_identity(&token) -> (token_key, username)`，创建 `info_span!(target: "kv", "kv::<method>", token_key, username, ...fields)`，内部逻辑块在 `.instrument(span)` 内执行。DB 操作以 `debug!`/`warn!`/`error!` 输出，target 为 `"kv"`（`auth.rs:138`、`auth.rs:144` 有两处 stray target `"auth"`，用于 token 有效性告警）。
...
| `get_value` | `token, namespace, key` | `Kv::Read("*")` 或 `Kv::Read(key)` 于 `Scope::KvNamespace(namespace)` | `get_v_from_kv`；返回 JSON 值，key 缺失返回字面量 JSON `null`（**非错误**）。命名空间不存在则报错。 |
| `get_multi_value` | `token, namespace_key: Vec<NamespaceKeyItem>` | `Kv::Read`（全局或逐项 精确/模式）于各项 `Scope::KvNamespace`；**全部项须通过，否则整体失败** | 拒绝空输入；逐项 `check_kv_read_permission_with_pattern`。精确键走 `get_v_from_kv_lenient` 单行查询（缺失为 null）；通配项每命名空间载一次 `get_kv_store_optional`（缓存于局部 map），按 `starts_with(prefix)` 过滤、排序、逐匹配产出 `KvValueItem`。请求顺序保留，通配匹配内排序。 |
| `set_value` | `token, namespace, key, value: Value` | `Kv::Write`（全局或特定 key）于 `Scope::KvNamespace(namespace)` | `set_v_to_kv` upsert（ON CONFLICT (namespace,key) DO UPDATE value）；返回 `{"success":true}`；命名空间须已存在。 |
| `delete_key` | `token, namespace, key` | `Kv::Delete`（全局或特定 key） | `delete_key_from_kv`；返回 `{"success":true}`。 |
| `delete_namespace` | `token, namespace` | `Kv::Delete("*")` **特定**于 `Scope::KvNamespace(namespace)` | `delete_kv`（删该命名空间全部行含 marker）；返回 `{"success":true}`。 |
| `get_all_keys` | `token, namespace` | `Kv::ListAllKeys` 于 `Scope::KvNamespace(namespace)` | `get_keys_from_kv`；返回升序 `Vec<String>`，**含** `NAMESPACE_MARKER_KEY`。 |
...
- **特性门控**：`default = []` 仅暴露 `KVStore`（仅类型，agent 安全）；`server` 特性追加 `auth.rs`、`db.rs`、`rpc/`。`lib.rs` 经 `#[cfg(feature = "server")] pub use` 条件再导出 server 门控项。
...
- **日志 target**：一致为 `"kv"`（`auth.rs:138`、`auth.rs:144` 有两处 stray `"auth"` 用于 token 有效性告警）。级别：成功路径 `debug`，校验/权限拒绝 `warn`，`create_kv` 插入失败 `error`。
...
- **权限模型**：用 `Scope::KvNamespace` + `Permission::Kv(Kv::<op>("*" or key))`；`"*"` 为 KV 操作全局通配。
...
- **维护者必须**保留“删整个命名空间需特定 `Kv::Delete("*")`”的严格性：即便 token 对每个 key 都有 `Kv::Delete(specific_key)`，也无法删命名空间。`crates/ng-kv/src/auth.rs:377`。
- **维护者必须**在任何触碰 DB 的命名空间处理代码前调用 `validate_namespace`（或会调它的 `check_*_permission`）：`validate_namespace` 拒绝空与含 `*` 的命名空间，因 `*` 是全局通配。`crates/ng-kv/src/auth.rs:41`。
- **切勿**假设 key 非空：`validate_key` 拒 `*` 但允许空串（测试 `validate_key_valid_empty` 确认有意为之）。`crates/ng-kv/src/auth.rs:63`。
- **维护者必须**在新增 KV 权限函数时择一选用 token 解析路径：`resolve_kv_list_namespace_permission` 与 `check_kv_create_permission` 用 `require_permission_checker()`（注入的 `PermissionChecker`），而 read/write/delete/list-keys 用 `ng_token::` 直接。两条路径都须在 `server/src/serve.rs` 注册；误配可绕过注入 checker 或导致编译失败。`crates/ng-kv/src/auth.rs:469`。
...
- **切勿**依赖通配匹配内的插入/扫描顺序：`get_multi_value` 请求顺序跨项保留，但单个通配请求内匹配键经 `sort_unstable` 按字典序输出。`crates/ng-kv/src/rpc/get_multi_value.rs:19`。

## 依赖关系

ng-kv 启用 `server` 特性后依赖：`ng-core`（`TokenOrAuth`、`Permission`、`Scope`、`Kv`、`PermissionChecker` trait、`NodegetError`/`error_code`）、`ng-db`（`kv` 实体、`get_db()` 全局单例、`DatabaseConnection`、迁移）、`ng-token`（`check_super_token`、`get_token`）、`ng-infra`（`rpc_exec!` 宏、`RpcHelper`、`token_identity`、`require_permission_checker`/`get_checker`）、以及 `jsonrpsee`、`sea-orm`、`serde`/`serde_json`、`tracing`、`anyhow`、`chrono`（`get_local_timestamp_ms_i64`）。默认特性下仅暴露 `KVStore`，agent 可仅依赖本 crate 的类型而不引入 server 门控。本 crate 被 server 二进制（`server/src/rpc_nodeget.rs::build_modules` 调 `rpc_module()`）与文档/工具链消费；agent 不启用其 `server` 特性。
