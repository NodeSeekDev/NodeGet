...
> 概览：ng-terminal 是一个**仅服务器端**的 WebSocket 中继，把 User 浏览器里的终端桥接到远端 Agent 的 shell。它**不是** JSON-RPC；只暴露一个 axum HTTP 路由 `/terminal`，在该路由上升级 WebSocket。Agent 先连接（携带 `task_token`+`task_id`+`terminal_id`），以 `(agent_uuid, terminal_id)` 为 key 注册一个 `SessionSlots` 句柄；User 随后连接（携带 `token`+`terminal_id`），经过 `PermissionChecker`（`Terminal::Connect` scope）授权后取走 Agent slot 的 receiver 半边，拼成一条双向管道。所有实际功能都在 `server` feature 之后；默认 features 下 crate 不导出公共 API，也不会编译 `ng-terminal` 自身模块。
...
- `lib.rs:11` 声明了三个 `#[cfg(feature = "server")] mod ...;`（`auth` / `check_agent` / `terminal`），仅在 server feature 启用时编译；默认 features 下不会编译 ng-terminal 自身模块。
...
- **Feature gate 模式（与其它 crate 不同）**：`ng-core` 是**唯一**无条件依赖（始终可用）；其它所有 crate 与外部依赖（`axum`、`tokio`、`serde`、`uuid`、`tracing`、`sea-orm`、`ng-db`、`ng-infra`、`ng-task`）都是可选的，全部位于 `server` feature 之后。默认 features 下 crate **什么都不导出**，且 `auth` / `check_agent` / `terminal` 这些模块不会编译；`lib.rs` 本身仍存在，只是用 item-level `#[cfg(feature = "server")]` 守卫模块与重导出。
...
- 错误信号约定：在 `handle_agent` / `handle_user` 已经拿到 `Query<TerminalParams>` 并走到显式拒绝分支时，通常会先 `Message::Text` 发送一个由 `generate_error_message(code, &msg)` 生成的 JSON 错误体再返回。常见码有硬编码的 `108=Invalid Input`、`102=Permission Denied`，也可能透传 `NodegetError` 的 `101/103/...` 等错误码（例如 Agent 侧校验报错）。这层逻辑**不**显式发起 close-handshake——socket 直接被 drop；但若 axum 的 Query 提取先失败（如缺少必填 `agent_uuid`），则不会进入这里的 handler 级 JSON 拒绝约定。
- UUID 解析约定：用 `Uuid::parse_str`；`handle_agent` 中失败走 `reject_with_error`，`auth`/`check_agent` 中返回 `NodegetError::ParseError`，`handle_user` 内部保留了一个本地 `warn+return` 的防御性解析分支。
...
- `std::sync::RwLockWriteGuard` 不是 `Send`，**不得**跨 `.await` 持有（`crates/ng-terminal/src/terminal.rs:228-236`）。所有 map 变更都在 `{ ... }` 同步块内（如 `231-248`、`299-305`、`389-403`、`469-475`）。在持写锁时加 `.await` 会编译失败（或 parking_lot 下死锁）；保持 map 访问在紧凑的同步作用域内。
- 锁中毒恢复（`crates/ng-terminal/src/terminal.rs:233-236`）：每次写都用 `.unwrap_or_else(std::sync::PoisonError::into_inner)`，意味着某连接里的 panic 中毒了锁会被静默吞掉——sessions 可能处于不一致状态但服务器继续运行。调试 "丢失" sessions 时应考虑先前是否有 panic 中毒了锁。
- `task_id` 是 `u64` 但 `task::Column::Id` 是有符号列，故用 `.cast_signed()`（`crates/ng-terminal/src/check_agent.rs:45-46`）。这个转换发生在 Rust 侧再进入 SQL 过滤；若调用方传入大于 `i64::MAX` 的 task_id，会被按位重解释成负的 `i64`，通常不会匹配正常自增 task id。实践中 task ID 很小，但若 ID 类型变宽，此 cast 仍是个 foot-gun。
- `check_agent` 用 `TaskEventResult.is_null()` 作为 "未完成" 谓词（`crates/ng-terminal/src/check_agent.rs:48-49`）。已产生结果的任务（终端附着过一次后再试）会不满足该过滤、返回 `Ok(false)` → "Permission Denied: Invalid Task Token or ID"。**复用已完成的 WebShell task id 开新终端 session 会被拒绝**——服务器必须为每个终端 session 生成新 task。
- `auth.rs` 与 `check_agent.rs` 依赖 `require_permission_checker()` 和 `ng_db::get_db()`——都是 OnceLock 全局，仅由服务器二进制在启动时初始化（`crates/ng-terminal/src/auth.rs:43`）。在未设置处调用 `check_terminal_connect_permission` 或 `check_agent` 会返回 `Err`（如 `DatabaseError 'DB not initialized'` 或 checker 未设置错误）。这些函数只有经服务器挂载的 router 触达才有意义。
- Agent 与 User **仅靠** query 中同时存在 `task_token` 与 `task_id` 来区分（`handle_socket` `terminal.rs:141-154`）。User 若误在 query 里带了 `task_token`+`task_id` 会被当 Agent 路由，命中 `check_agent`（会被拒绝，但永不到达 User 权限路径）。**保持** User 客户端的 query 不带这两个参数。
- User 侧在 `terminal_id` 和 `token` 都具备时，若 `agent_uuid` 非法，当前流程会先在 `check_terminal_connect_permission` 里解析失败并走统一的 102 `Permission Denied: Terminal connection permission denied` JSON 响应（`crates/ng-terminal/src/auth.rs:37-38`，`crates/ng-terminal/src/terminal.rs:359-369`），而不是静默关闭。`handle_user` 里后面的本地 `Uuid::parse_str` + `warn+return`（`terminal.rs:381-383`）在现有授权顺序下更像防御性兜底分支。
