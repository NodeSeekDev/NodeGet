...
> 概览：ng-token 是 NodeGet 的 RBAC 实现层。在 `server` feature 下，它持有全表内存 token 缓存（`TokenCache`，基于 `DbBackedCache` + `make_global_cache!`），负责 super token（ID=1）的生成/轮换/校验、子 token 创建、scope+permission 覆盖判定（仅后缀通配），以及 `token_*` JSON-RPC 命名空间的 CRUD。认证时，super token 的 key/username 与 secret/password hash 使用 `ct_eq`；普通 token 则先经缓存索引按 key/username 定位，再对 secret/password hash 做 `ct_eq` 校验。未启用 `server` 时 crate 仅再导出轻量错误/权限 API（含 `anyhow_to_nodeget_error`）；当前 agent 并不直接依赖本 crate。server 侧 trait 注入里，直接委派到 ng-token 的是 `PermissionChecker`；`JsWorkerService`、`JsWorkerScheduler`、`MonitoringUuidProvider` 分别落在其他 crate。

## 模块结构

```
crates/ng-token/src/
├── lib.rs              # Crate root：再导出轻量错误/权限 API；server feature 下暴露 hash 助手、模块与 rpc_module()
├── cache.rs            # 全表内存缓存 TokenCache + SUPER_TOKEN_GLOBAL，缓存索引查询 + ct_eq hash 校验
...
| 名称 | 签名 | 行为 |
|------|------|------|
| 轻量再导出 | `pub use ng_core::permission::data_structure::{Limit, Permission, Scope, Token}; pub use ng_core::permission::token_auth::TokenOrAuth; pub use ng_core::error::{NodegetError, anyhow_to_nodeget_error};` (`lib.rs:18-20`) | 未启用 `server` 时唯一可见的 pub 项：错误/权限相关轻量 API。 |
...
| `TokenCache` | `pub struct TokenCache; impl DbBackedCache for TokenCache;` | 全表 token 缓存单例。查询：`find_by_key` / `find_by_username` / `get_super_token` / `get_all`；认证：`authenticate`（super-token 标识用 `ct_eq`，普通 token 先走 `HashMap` 索引再比较 hash）。生命周期由 `make_global_cache!` 生成 `init()` / `global()` / `reload()`（`cache.rs:82`）。 |
...
| `get_token` | `pub async fn get_token(token_or_auth: &TokenOrAuth) -> anyhow::Result<Token>` (`get.rs:42`) | 缓存索引定位 + `ct_eq` 校验 secret/password hash，返回不含 hash 字段的 `Token`；失败统一 `AUTH_FAILED_MESSAGE`。 |
...
- 凭据生成长度：`token_key = generate_random_string(16)`；子 token 的 `token_secret = generate_random_string(32)`；super token 的 root password 也随机 32 字符；`token_create` 的普通 `password` 为调用方提供，不自动生成。
...
`check_super_token`（`super_token.rs:168`）先检查 `TokenCache::global().is_none()`，返回 `ConfigNotFound`（而非误导性的 `NotFound`）；再读 `get_super_token()`，对 super token 的 Token 变体用 `ct_eq` 比较 `(key, secret-hash)`、Auth 变体用 `ct_eq` 比较 `(username, password-hash)`。部分不匹配返回 `Ok(false)`，全匹配返回 `Ok(true)`。`authenticate()`（`cache.rs:282`）镜像该 super-token 快路径；普通 token 路径则先经 `find_by_key` / `find_by_username` 的 `HashMap` 查找条目，再对 secret/password hash 做 `ct_eq` 校验，而不是对普通 `token_key` / `username` 做常量时间匹配。当 key 匹配 super 但 secret 不匹配时它会**立即返回失败**，不再扫 `by_key`——因为 super key 全局唯一，重扫只会重命中同一 super 条目并多烧一次 SHA256；这让“错误 super secret”与“错误普通 secret”两条路径的 SHA256 次数一致。所有失败返回 `NodegetError::PermissionDenied("Invalid credentials")`。
...
ng-token 由 `ServerPermissionChecker`（在 `serve.rs` 注册）直接消费，后者将 `check_token_limit` / `check_super_token` / `get_token` 委派到本 crate。其余注入 trait 并不由 ng-token 提供：`TaskMonitoringUuidProvider` 走 `ng_monitoring::MonitoringUuidCache`，`JsWorkerServiceImpl` / `CronJsWorkerScheduler` 分别走 `ng_js_worker` 与 RPC 模块装配。

## RPC 方法

命名空间 `token`（分隔符 `_`）。鉴权由 handler 内部显式完成，框架层无鉴权。多数管理方法把调用方凭据放在首个参数；`token_get` 例外：首参是目标 token/凭据，管理模式凭据在第二个参数 `supertoken`。新增方法必须显式选择合适守卫（`get_token` / `check_super_token` / `check_token_limit`），否则等同公开未鉴权。
...
namespace 不受 jsonrpsee 框架保护。管理类方法通常把调用方凭据放在首参并在方法体内调用 `check_super_token`；`token_get` 自查模式对首参 `token` 调 `get_token`，管理查询模式则对第二参 `supertoken` 调 `check_super_token`。若未来有需要细粒度 RBAC 的新方法，应显式调用 `get_token` / `check_super_token` / `check_token_limit` 中的合适守卫，否则该方法就是公开未鉴权的。
...
- **Edition 2024**：全程使用 let-chains（如 `get.rs:364`、`generate_token.rs:76`）。
- **Feature gates**：`default = []` 仅再导出轻量错误/权限 API（不含 server 逻辑）；`server` feature 门控 `cache` / `generate_token` / `get` / `rpc` / `super_token` 模块、`hash_string` / `hash_to_bytes`，以及 `rpc_module()`。
...
- **日志 target 约定（非穷举）**：领域日志主要用 `"token"` 管理操作、`"auth"` 认证、`"token_cache"` 缓存锁/poison 事件；`make_global_cache!` / `rpc_exec!` 还会打到 `"cache"` / `"rpc"`。
- **哈希**：SHA256，前置固定盐 `b"NODEGET"`（`lib.rs:70`）；token_secret 与 password 同算法。DB 存 hex，缓存存原始 `[u8;32]`。
- **常量时间比较**：super token 的 `token_key` / `username` 与所有 secret/password hash 都走 `ct_eq`；普通 token 的 `token_key` / `username` 通过 `HashMap` 索引定位后，再比较 hash。
...
- **新建 RPC 方法切勿遗漏鉴权**（`rpc/mod.rs:41`）：`token` namespace 的鉴权全靠 handler 显式处理；`token_get` 自查走 `get_token(token)`，管理查询走第二参 `supertoken` + `check_super_token`，其余管理方法通常把调用方凭据放在首参。新增方法至少要显式调用 `get_token` / `check_super_token` / `check_token_limit` 之一；遗漏即公开未鉴权。

## 依赖关系

ng-token 位于 RBAC 的核心，向内依赖 ng-core（`Token` / `Limit` / `Scope` / `Permission` / `TokenOrAuth` 类型、`NodegetError` 与 `anyhow_to_nodeget_error`、`generate_random_string`、`get_local_timestamp_ms_i64` 等 utils）与 ng-infra（`DbBackedCache` trait 与 `make_global_cache!` 宏、`rpc_exec!`、`token_identity`），并在 `server` feature 下引用 ng-db 的 `token` 实体。向外，server binary 通过 `ServerPermissionChecker` 直接委派 `check_token_limit` / `check_super_token` / `get_token` 到本 crate；其他注入 trait（`TaskMonitoringUuidProvider`、`JsWorkerServiceImpl`、`CronJsWorkerScheduler`）分别委派到 `ng_monitoring` / `ng_js_worker` / RPC 模块装配，并不经过 ng-token。其他调用方若只需共享错误/权限类型，可仅用默认 feature；当前 agent 不直接依赖 ng-token。
