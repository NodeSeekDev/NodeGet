...
> 概览：ng-static 负责管理 NodeGet 的静态文件 bucket（数据库表 `static_file` 中 bucket name → 磁盘子目录的映射），通过 HTTP（`/nodeget/static/{name}`）对外提供文件下载，并提供 WebDAV 端点（`/nodeget/static-webdav/{*path}`）和两个 JSON-RPC 命名空间（`static-bucket` 做 bucket CRUD，`static-bucket-file` 做文件 upload/read/delete/rename/list）。所有写操作触发全表内存缓存（StaticCache，基于 `DbBackedCache` + `make_global_cache!`）刷新；所有磁盘路径锚定在可配置的 `static_path`（默认 `./static/`）之下，并通过 `validate_name`/`validate_sub_path`/`resolve_safe_file_path` 三件套防御目录穿越。
...
| `ops.rs` | 核心业务逻辑：bucket CRUD、文件 CRUD、path 安全校验、`static_path` 配置缓存。所有写均调用 `StaticCache::reload()` |
| `router.rs` | axum Router：`/nodeget/static/{name}[/{*path}]` 静态文件 + `/nodeget/static-webdav/{*path}` WebDAV；DavHandler 按 bucket 缓存；ETag/If-None-Match、MIME 猜测、CORS、index.html fallback、nosniff |
...
| `serve_static_file` | `router.rs:190` | `async fn(sub_path, path, cors, method, if_none_match) -> Response<HttpBody>`：仅 GET/HEAD（其它→405 带 Allow）；`resolve_safe_file_path`；目录→index.html fallback；弱 ETag（`mtime_secs-size`）；If-None-Match 命中→304；设置 Content-Type、ETag、`X-Content-Type-Options:nosniff`、可选 CORS `ACAO:*`；HEAD 不带 body |
...
| `auth::check_super_token` | `rpc/static_bucket/mod.rs:14` | `pub async fn check_super_token(token: &str) -> anyhow::Result<bool>`：解析 token、取 checker、`check_super_token`；解析失败或缺少 checker→Err |
...
`resolve_safe_file_path`（`ops.rs:174`）防御性地再校验 sub_path，然后遍历分量：`Normal` push、`RootDir`/`CurDir` 跳过、`ParentDir` pop（若会逃逸 base 则报错）、`Prefix` 拒绝。最后的 `starts_with(base)` 兜底符号链接/边界情况。`list_file` 的 `collect_files`（`ops.rs:652`）使用 `symlink_metadata` 并显式跳过符号链接文件类型，阻止恶意符号链接逃逸 bucket 根。
...
每个方法开启 `info_span!(target:"static_bucket[_file]", token_key, username, ...)`，运行 `async { rpc_exec!(<inner>().await) }.instrument(span).await`。内部 fn 返回 `anyhow::Result<Box<RawValue>>`；`rpc_exec!` + `anyhow_to_nodeget_error` 负责日志与 `ErrorObject` 映射（`ErrorObject::owned(code, msg, None::<()>)`）。
...
| `is_http_root` | `bool` | 全局唯一（应用层强制） |
| `cors` | `bool` | 是否对该 bucket 启用 CORS |
| `enable` | `Option<bool>` | `None` 或 `Some(false)` 视为禁用——`Some(false)` 在 HTTP/WebDAV 路由返回 404 |

无 SeaORM relations。`is_http_root` 唯一性由 `ops::create_static`/`update_static` 查询强制（无 DB 级约束），因此并发 insert 理论上可能产生重复（`cache.build_maps` 保留首个并对重复 warn）。磁盘目录 `{static_path}/{path}` 在 create/update 时创建，但 delete 时**从不删除**（仅删 DB 行）。
...
- **内部 RPC fn**：使用 `process_logic` async 块返回 `anyhow::Result<Box<RawValue>>`，再经 `anyhow_to_nodeget_error` → `ErrorObject::owned(code, msg, None::<()>)` 映射。
...
- **Config hot-reload**：必须在 config reload 后调用 `reload_static_path()`；`update_static`/`delete_static` 自动调用 `clear_dav_handler_cache()` 以免 stale LocalFs 绑定。
...
- **`crates/ng-static/src/cache.rs:106`**：`is_http_root` 唯一性在应用层（ops 查询）强制，缓存层仅 warn 兜底（build_maps 保留首个）。无 DB 级唯一约束，两次并发 `create_static(is_http_root=true)` 可能竞态双双插入，缓存将静默只保留一个。**依赖调用方串行化写**。
...
- **`crates/ng-static/src/ops.rs:562`**：`list_file` 在 `spawn_blocking` 下用 `tokio::fs`（`collect_files` 同步）。通过 `symlink_metadata` 文件类型检查跳过符号链接；但 `resolve_safe_file_path` **不**作用于 list 结果——`collect_files` 直接遍历磁盘，仅靠 `is_file`/`is_dir`/`is_symlink` 与「base 为 bucket 根」保证安全。指向 bucket 外文件的非符号硬链接仍会被列出/读取（硬链接不可检测）。
