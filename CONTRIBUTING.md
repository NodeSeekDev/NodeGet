...
- 代码是否引入了新的依赖库？如有，新的依赖库是否被 Rust 社区广泛使用，是否不明显影响项目的构建时间和体积，是否已经最小化 Features，是否有同类小型库可替
- 是否同步更新文档说明
- 禁止任何地方代码在正常运行时，以任何形式退出程序执行环境
- 代码格式化，提交前在项目根目录运行 `cargo fmt`，以确保代码风格一致（不要使用 RustRover 等 IDE 工具自带的代码格式化工具）

所有不具有在主项目中 Push 到 dev 分支的开发者，都应先行 Fork 项目后，在自己的分支中的 dev 进行开发，完成后提交 PR 到主项目的 dev 分支。